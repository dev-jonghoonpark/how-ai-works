...
- **[dictionary.md](dictionary.md)**
  - 자료를 읽다 막히는 용어를 정리합니다. — 텐서 · 단방향/양방향 RNN(+ DS2의 행 합성곱이 왜 단방향일 때만 붙는가) · Post-LN/Pre-LN(+ 워밍업 4000스텝이 왜 필요했는가)
...
- **[Annotated Transformer Modern](https://github.com/dev-jonghoonpark/annotated-transformer-modern)** · 💻 내 컴퓨터에서 학습
  - How Transformer Works가 논문을 브라우저에서 뜯어 봤다면, 이쪽은 코드를 직접 돌려 보는 쪽 — [The Annotated Transformer](https://nlp.seas.harvard.edu/annotated-transformer/)(Rush 2018, Huang 외 2022 개정)가 고정한 torch 1.11·torchtext·torchdata가 지금은 설치조차 안 되는 것을 HF datasets + 공유 BPE로 고치고, 같은 모델을 **지금 오픈 LLM의 부품(RMSNorm·RoPE·SwiGLU·GQA·QK-norm·SDPA·KV 캐시)으로 다시 짜서** Multi30k 독→영으로 나란히 학습(RTX 3070 한 장)한다. 원본 코드가 논문과 다른 곳도 짚는다 — 튜토리얼 설명은 Post-LN인데 코드는 "단순화"라며 **Pre-LN**이고, 논문대로 Post-LN으로 되돌리면 나머지가 같아도 **BLEU 39.0 → 13.4**. 결과는 정직하게: 29,000문장 규모에서는 BLEU가 전부 37~39로 **seed 편차(1~2) 안**이라 현대 부품이 품질을 올려 주지는 않고, 대신 **학습 20.5분 → 9.3분, 메모리 5.86 → 3.50GiB**, 검증 손실 최저점 2,500 → 1,000스텝으로 빨라진다(그중 대부분은 구조가 아니라 bf16·fused CE 같은 **레시피** 몫). 인코더 없이 `<bos> 원문 <sep> 번역문`을 한 줄로 푸는 **디코더 전용**도 0.3~2 BLEU 차이로 따라오고, 요즘 LLM처럼 드롭아웃을 0으로 끄면 38 epoch를 도는 작은 데이터에서는 나빠진다. 검증 NLL은 1,000~2,500스텝에 최저를 찍고 오르는데 BLEU는 대부분 끝까지 돈 쪽이 높은 것(레이블 스무딩과 argmax), KV 캐시가 짧은 문장에서는 커널 실행 고정비에 묻혀 이득이 없다가 1,024토큰에서 **2.8배**가 되는 것(재계산 O(n²) vs 캐시 O(n)), GQA가 그 캐시를 토큰당 28KB → 7KB로 줄이는 것까지 숫자로 확인한다
